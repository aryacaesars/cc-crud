...
export default function Home() {
  ...
  // Formatter IDR deterministik agar SSR/CSR konsisten
  const formatIDR = (n: number): string => {
    const sign = n < 0 ? '-' : '';
    const abs = Math.abs(Math.trunc(n));
    const s = abs.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
    // Gunakan 'Rp ' dengan spasi konsisten seperti UI Anda
    return `${sign}Rp ${s}`;
  };

  // Helper function untuk format currency input (tanpa Intl)
  const formatCurrencyInput = (value: string): string => {
    // Hapus semua karakter non-digit
    const numberOnly = value.replace(/\D/g, '');
    if (!numberOnly) return '';
    return formatIDR(parseInt(numberOnly, 10));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editId) {
      ...
      showPopup('info', 'PERINGATAN!', 'Apakah Anda yakin ingin mengupdate transaksi ini?', doUpdate);
    } else {
      // Tambah transaksi
  ...
  const formatCurrency = (amount: number): string => formatIDR(amount);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
          <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xl md:text-2xl font-bold text-green-600">{formatCurrency(getTotalIncome())}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-xl md:text-2xl font-bold text-red-600">{formatCurrency(getTotalExpense())}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center ${getBalance() >= 0 ? 'bg-blue-100' : 'bg-orange-100'}`}>
                  <svg className={`w-5 h-5 md:w-6 md:h-6 ${getBalance() >= 0 ? 'text-blue-600' : 'text-orange-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="ml-4">
                ...
                <p className={`text-xl md:text-2xl font-bold ${getBalance() >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-sm border p-4 md:p-6 lg:sticky lg:top-8">
              ...
              <div className="border-b p-4 md:p-6">
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-medium text-xs md:text-sm transition-colors ${
                      filter === "all" 
                ...
                {loading ? (
                  ...
                  <div className="space-y-2.5 md:space-y-3">
                    {transactions.map(transaction => (
                      <div key={transaction.id} className="border rounded-lg p-3 md:p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              {transaction.description && (
                                <p className="text-xs md:text-sm text-gray-700">{transaction.description}</p>
                              )}
                          ...
                          <div className="text-right flex items-center space-x-3">
                            ...
                            <div className="flex space-x-1">
                              <button
                                className="p-2 md:p-1 text-gray-400 hover:text-blue-600 transition-colors"
                              ...
                              <button
                                className="p-2 md:p-1 text-gray-400 hover:text-red-600 transition-colors"
